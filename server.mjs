...
var playerNameOverride = undefined;
    ...
    app.get('/:room', function(req, res, next) {
        ensureRoomIsLoaded(req.params.room).then(function(isLoaded) {
            ...
            if (req.query.playerName) {
                Logging.log(`Received player name override ${req.query.playerName}`);
                playerNameOverride = req.query.playerName;
            }
        }).catch(next);
    });
...
const ws = new WebSocket(server, serverStart, function(connection, { playerName, roomID }) {
    ensureRoomIsLoaded(roomID).then(function(isLoaded) {
        if (isLoaded) {
            var player = new Player(connection, playerName, activeRooms.get(roomID));
            activeRooms.get(roomID).addPlayer(player);
        }
        if (playerNameOverride !== undefined && playerNameOverride !== null && playerNameOverride !== '') {
            activeRooms.get(roomID).renamePlayer(player, playerName, playerNameOverride);
        }
